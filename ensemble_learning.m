%   Скрипт  ensemble_learning.m
%   Автор   Роман Кошкин
%   Назначение: выполняет в режиме кросс-валидации построение решающих деревьев (decision trees) для предсказания относительного 
%   рейтинга аудиосообщения.
%   Параметры:  список параметров указывается в начале скрипта и определяет поведение скрипта. 
%   Параметры:
%   elect_subj              -   массив ячеек, содержащий имена испытуемых\респондентов, иднтифицирующих конкретную ЭЭГ запись
%   ads                     -   вектор, содержащий номера используемых рекламных сообщений. Примечание: номера в этом примере указываются в 
%   соответствии с нумерацией в БД, используемой разработчиками
%   suppress_ERP            -   значение 1 соответствует режиму подавления вызванного ответа при построении деревьев.
%   zero_mean_unit_var      -   значение 1 соответствует режиму стандартизации аннкетных данных вниути каждого испытуемого
%   collect_data_for_XGBoost-   значение 1 соответствует режиму сохранения данных для последующего использования алгоритмом градиентным 
%                               бустингом
%   plotgraphs              -   значение 1 соответствует режиму отрисовки графиков
%   passband_cap            -   верхняя частота в спектре сигнала, используемая для формирования признаков = 40
%   MaxNumSplits            -   максимальное значение ветвлений дерева  
%   number_of_frequencies   -   число узкополосных компонент, формирующих вектор признаков
%   ChannelsOfInterest      -   канал, используемый для предсказания 
%   QuestionsOfInterest     -   номер вопроса рейтинг которого предсказывается. 
%   n_test                  -   количество исключаемыз реклам при валидации
%   boosting                -   значение 1 соответствует выполнению градиентного бустинга. 
%   
%   В результате выполнения скрипта генерируется таблица, в переменной tab, содержащая информацию для каждого исключенного на
%   последовательных этапах рекламного сообщения. 
%   Сожержание толбцов таблицы:
%   across_subj_corr -  значения коэффициента корреляции пирсона между предсказанным рейтингом рекламного сообщения и рейтингом,
%   выведенным на основе анкеты по множеству респондентов.
%   GT               -  средний по респондентам рейтинг на основе анкеты.
%   pred             -  средний по респондентам рейтинг, предсказанный алгоритмом 
%   Возвращаемые переменные:    нет
...
%   Функция  [PXX, f, z, ad] = get_set(EEG, ads, quest, chan, zero_mean_unit_var, number_of_frequencies, elect_subj)
%   Автор   Роман Кошкин
%   Назначение: Извлекает записи ЭЭГ данных канала <chan> респондентов <elect_subj>  во ремя прослушивания сообщений <ads> и вычисляет 
%   информативные признаки на основе периодограммы (частотное разложение). Формирует пару [признаки <PXX> - указание учителя <z>]. 
%   Указание учителя совпадает со средним рейтингом рекламы по выбранному вопросу <quest>, полученным на основе анкеты. 
%   По требованию выполняет стандартизацию распределения указаний учителя.  
%   Параметры:  
%   EEG                     - структура  EEGLab, содержащая ЭЭГ данные и метаданные
%   ads                     - вектор, содержащий порядковые номера в базе данных интересующих рекламных сообщений
%   quest                   - вектор, содержащий порядковые номера вопросов в анкете
%   chan                    - скаляр, номер ЭЭГ канала, который используется для извлечения признаков.
%   zero_mean_unit_var      - флаг стандартизации указаний учителя
%   number_of_frequencies   - число отсчетов частоты, для которых вычисляется периодограмма  
%   elect_subj              - ветор с индексами пациентов, для которых должны быть извлечены признаки
%   Возвращаемые переменные:
%   PXX                     - массив, содержащий информационные признаки
%   f                       - вектор значений частот, для которых рассчитаны информативные признаки (мощность)
%   z                       - вектор указаний учителя
%   ad                      - порядковый номер рекламного сообщения 
